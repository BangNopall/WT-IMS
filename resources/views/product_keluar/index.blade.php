...
@section('content')
    <div class="box box-success">
        ...
        <div class="box-header">
            <h3 class="box-title">WD Barang</h3>
        </div>

        <div class="box-header">
            <a onclick="addForm()" class="btn btn-success" ><i class="fa fa-plus"></i> Tambahkan WD Barang</a>
            <a href="{{ route('exportPDF.wdbarangAll') }}" class="btn btn-danger"><i class="fa fa-file-pdf-o"></i> Export PDF</a>
            <a href="{{ route('exportExcel.wdbarangAll') }}" class="btn btn-primary"><i class="fa fa-file-excel-o"></i> Export Excel</a>
        </div>
            ...
            <table id="invoice" class="table table-bordered table-hover table-striped">
                ...
                @foreach($invoice_data as $i)
                    <tbody>
                        ...
                        <td>{{ $i->tanggal }}</td>
                        <td>
                            <a href="{{ route('exportPDF.wdbarang', [ 'id' => $i->id ]) }}" class="btn btn-sm btn-danger">Export Invoice</a>
                        </td>
                    </tbody>
                @endforeach
            </table>
...
@section('bot')
    ...
    <script type="text/javascript">
        ...
        function addForm() {
            ...
            $('.modal-title').text('Add WD Barang');
        }
        ...
        $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('wdbarang') }}";
                    else url = "{{ url('wdbarang') . '/' }}" + id;
